...
def generate_boxplot_pdf(df_anova, metabolites):
    """Generate a PDF with 4 boxplots per page (2x2 grid) using reportlab and kaleido.

    Parameters
    ----------
    df_anova : pd.DataFrame
        The ANOVA result dataframe (used by get_metabolite_boxplot).
    metabolites : list
        Ordered list of metabolite identifiers to plot.
    ...
    for i, metabolite in enumerate(metabolites):
        ...
        # Start a new page for every group of 4 (but not for the very first)
        if pos == 0 and i > 0:
            c.showPage()

        fig = get_metabolite_boxplot(df_anova, metabolite)
