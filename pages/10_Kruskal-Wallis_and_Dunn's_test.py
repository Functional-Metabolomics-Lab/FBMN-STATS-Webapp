    ...
    with top_tabs[0]:
        ...
        if st.session_state.df_kruskal is not None and not st.session_state.df_kruskal.empty:
            ...
            with kw_sub_tabs[2]:
                ...
                def _kw_clear_pdf_on_mode_change():
                    st.session_state.pop("kw_boxplot_pdf_bytes", None)
                    st.session_state.pop("kw_boxplot_pdf_label", None)

                _kw_pdf_mode = st.radio(
                    "Selection mode",
                    options=["Top N significant", "Top N insignificant", "Single metabolite"],
                    horizontal=True,
                    key="kw_boxplot_pdf_mode",
                    on_change=_kw_clear_pdf_on_mode_change,
                )

                if _kw_pdf_mode in ("Top N significant", "Top N insignificant"):
                    _kw_df_check = st.session_state.df_kruskal
                    _kw_want_sig = (_kw_pdf_mode == "Top N significant")
                    if "significant" in _kw_df_check.columns:
                        _kw_avail = int(_kw_df_check["significant"].eq(_kw_want_sig).sum())
                    else:
                        _kw_avail = len(_kw_df_check)
                    _kw_max_n = max(5, min(20, _kw_avail))
                    _kw_min_n = min(5, _kw_avail)
                    if _kw_avail == 0:
                        st.warning(f"No {'significant' if _kw_want_sig else 'insignificant'} metabolites available.")
                        _kw_top_n = 0
                    else:
                        _kw_top_n = st.slider(
                            ...
                            min_value=_kw_min_n,
                            max_value=_kw_max_n,
                            value=min(10, _kw_max_n),
                            step=1,
                            key="kw_boxplot_pdf_topn",
                        )
                        if _kw_avail < 20:
                            st.caption(f"{_kw_avail} {'significant' if _kw_want_sig else 'insignificant'} metabolites available.")
                else:
                    _kw_top_n = 1

                if st.button("Generate PDF", key="kw_generate_pdf_btn", type="primary"):
                    _kw_df = st.session_state.df_kruskal
                    _kw_p_col = next((c for c in ["p-corrected", "p"] if c in _kw_df.columns), None)
                    if _kw_pdf_mode == "Single metabolite":
                        _kw_mets = [st.session_state.kruskal_metabolite]
                        _kw_label = f"single_{st.session_state.kruskal_metabolite}"
                    else:
                        if "significant" not in _kw_df.columns:
                    ...
                    if _kw_mets:
                        with st.spinner(f"Generating PDF — {len(_kw_mets)} boxplot(s)…"):
                            _kw_pdf = generate_boxplot_pdf_generic(_kw_df, _kw_mets, get_metabolite_boxplot)
                        st.session_state["kw_boxplot_pdf_bytes"] = _kw_pdf
                        st.session_state["kw_boxplot_pdf_label"] = _kw_label
                    else:
                        st.warning("No metabolites to include in the PDF.")
                        st.session_state.pop("kw_boxplot_pdf_bytes", None)

                if st.session_state.get("kw_boxplot_pdf_bytes"):
                    st.download_button(
                        label="⬇ Download PDF",
                        data=st.session_state["kw_boxplot_pdf_bytes"],
                        file_name=f"kruskal_{st.session_state.get('kw_boxplot_pdf_label', 'boxplots')}.pdf",
                        mime="application/pdf",
                        key="kw_download_pdf_btn",
                    )
